...
- 2026-05-30 post-quota-reset browser retest:
  - Raised the Live QA workspace monthly query limit from 25 to 1000 to continue source QA without deleting usage history.
  - Retested `Was regelt Paragraf 33 FinStrG zur Abgabenhinterziehung? Bitte mit Quellen.` through the live UI. The source panel now includes `§ 33 Finanzstrafgesetz` and opens the exact direct RIS `NormDokument.wxe` URL with `Gesetzesnummer=10003898&Artikel=1&Paragraf=33`; RIS rendered `Finanzstrafgesetz Art. 1 § 33`, not a search/overview page.
  - Retesting `Was regelt Paragraf 16 MRG zur Mietzinsbildung? Bitte mit Quellen.` exposed a bad hardcoded MRG RIS law number (`10002521`, textile labelling regulation). Corrected backend and frontend mappings to `10002531`, deployed `retrieval`, and reran the UI test. The source panel now opens direct `Mietrechtsgesetz § 16` via `NormDokument.wxe`.
  - `Was regelt Paragraf 1 KSchG? Bitte mit Quellen.` opened the exact direct RIS `NormDokument.wxe` URL for `Konsumentenschutzgesetz § 1`.
  - Regression retest `Was ist Mord nach Paragraf 75 StGB? Bitte mit Quellen.` now exposes the exact source-card URL for `Strafgesetzbuch § 75`; opening it lands on the direct RIS `NormDokument.wxe` document, not a generic search result.
